...
def import_covid_zahlen():
    ...
    next(data)
    for row in data:
        #thread.append(Parser(sql, row, lock, threadLimiter).start())
        sql = sql+"('"+row[0]+"','"+row[1]+"',"+str(row[2])+","+str(row[3])+","+str(row[4])+","+str(row[5]) + \
...
def import_impfzahlen():
    data = csv.reader(StringIO(requests.get(
        "https://impfdashboard.de/static/data/germany_vaccinations_timeseries_v2.tsv").text))
    ...
    for row in data:
        tsv = row[0].split('\t')
        sql = sql+"('"+tsv[0]+"','"+tsv[1]+"',"+str(tsv[2])+","+str(tsv[3])+","+str(tsv[4])+","+str(tsv[5])+","+str(tsv[6])+","+str(tsv[7])+","+str(tsv[8])+","+str(tsv[9])+","+str(tsv[10])+","+str(tsv[11])+","+str(tsv[12])+","+str(tsv[13])+","+str(tsv[14])+","+str(tsv[15])+","+str(tsv[16])+","+str(tsv[17])+","+str(tsv[18])+","+str(tsv[19])+","+str(tsv[20])+","+str(tsv[21])+","+str(tsv[22])+","+str(
            tsv[23])+","+str(tsv[24])+","+str(tsv[25])+","+str(tsv[26])+","+str(tsv[27])+","+str(tsv[28])+","+str(tsv[29])+","+str(tsv[30])+","+str(tsv[31])+","+str(tsv[32])+","+str(tsv[33])+","+str(tsv[34])+","+str(tsv[35])+","+str(tsv[36])+","+str(tsv[37])+","+str(tsv[38])+","+str(tsv[39])+","+str(tsv[40])+","+str(tsv[41])+","+str(tsv[42])+","+str(tsv[43])+","+str(tsv[44])+","+str(tsv[45])+"),"
    sql = re.sub(r'.$', ';', sql)
    execute_sql(sql)
...
def import_impf_lieferung():
    data = csv.reader(StringIO(requests.get(
        "https://impfdashboard.de/static/data/germany_deliveries_timeseries_v2.tsv").text))
...
def import_cwa():
    data = requests.get(
        "https://obs.eu-de.otc.t-systems.com/obs-public-dashboard/json/v1/nested_cwa_public_dashboard_data.json").json()
    ...
    for row in data[0]['data']['daily']:
        sql = sql+"('"+row[0]+"','"+row[1]+"',"+str(row[2])+","+str(row[3])+","+str(row[4])+","+str(row[5])+","+str(row[6])+","+str(row[7])+","+str(row[8])+","+str(row[9])+","+str(row[10])+","+str(row[11])+","+str(row[12])+","+str(row[13])+","+str(row[14])+","+str(row[15])+","+str(row[16])+","+str(row[17])+","+str(row[18])+","+str(row[19])+","+str(row[20])+","+str(row[21])+","+str(row[22])+","+str(row[23])+","+str(row[24])+","+str(row[25])+","+str(row[26])+","+str(row[27])+","+str(row[28])+","+str(row[29])+","+str(row[30])+","+str(row[31])+","+str(row[32])+","+str(row[33])+","+str(row[34])+","+str(row[35])+","+str(row[36])+","+str(row[37])+","+str(row[38])+","+str(row[39])+","+str(row[40])+","+str(row[41])+","+str(row[42])+","+str(row[43])+","+str(row[44]) + \
            ","+str(row[45])+","+str(row[46])+","+str(row[47])+","+str(row[48])+","+str(row[49])+","+str(row[50])+","+str(row[51])+","+str(row[52])+","+str(row[53])+","+str(row[54])+","+str(row[55])+","+str(row[56])+","+str(row[57])+","+str(row[58])+","+str(row[59])+","+str(row[60])+","+str(row[62])+","+str(row[63])+","+str(row[64])+","+str(row[65])+","+str(row[66])+","+str(row[67]) + \
            ","+str(row[68])+","+str(row[69])+","+str(row[70])+","+str(row[71])+","+str(row[72])+","+str(row[73])+","+str(row[74])+","+str(row[75])+","+str(row[76])+","+str(row[77])+","+str(row[78]) + \
            ","+str(row[79])+","+str(row[80])+","+str(row[81])+","+str(row[82])+","+str(row[83])+","+str(
                row[84])+","+str(row[85])+","+str(row[86])+","+str(row[87])+","+str(row[88])+","+str(row[89])+"),"
    sql = re.sub(r'.$', ';', sql)
...
def import_kh():
    data = requests.get(
        "https://www.intensivregister.de/api/public/intensivregister").json()
    for object in data['data']:
        ...
        kh = Krankenhaus(object['krankenhausStandort']['id'],
                         ...
                         str(object['krankenhausStandort']['position']['latitude']) +
                         "/"+str(object['krankenhausStandort']
                                 ['position']['latitude']),
...
def csv_import_landkreis():
    data = csv.reader(StringIO(requests.get(
        "https://diviexchange.blob.core.windows.net/%24web/zeitreihe-tagesdaten.csv").text))
    # Skip CSV Header
    next(data)
    ...
    for row in data:
        sql = sql+"('"+row[0]+"',"+row[1]+","+row[2]+","+row[3]+","+row[4] + \
            ","+row[5]+","+row[6]+","+row[7]+"," + \
            row[8]+","+row[9]+","+row[10]+"),"
    sql = re.sub(r'.$', ';', sql)
    execute_sql(sql)
